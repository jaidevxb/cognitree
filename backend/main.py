...
if GEMINI_API_KEY:
    print(f"DEBUG: GEMINI_API_KEY loaded (length: {len(GEMINI_API_KEY)})")
...
def generate_content_with_retry(model, prompt, retries=3, initial_delay=1):
        ...
        try:
            ...
            if "429" in str(e) or "quota" in str(e).lower():
                if attempt < retries - 1:
                    ...
                    print(f"DEBUG: Quota exceeded, retrying in {sleep_time:.2f}s... (Attempt {attempt+1}/{retries})")
...
async def search(request: SearchRequest):
    ...
    try:
        ...
        prompt = f"""
        What are the primary 5 areas in {request.query}? 
        Please provide exactly 5 distinct areas, each with a brief description and a relevant search query for further research. 
        ...
        # print(f"DEBUG: Response text: {response.text}") # Commented out to avoid clutter, uncomment if needed
        ...
        try:
            ...
            # Fallback to generic results if JSON parsing fails
        ...
        if structured_data and "areas" in structured_data:
            ...
            print("DEBUG: Using fallback results")
            ...
            for i in range(5):
                results.append({
                    ...
                    "llm_content": response.text if hasattr(response, 'text') else "Error generating content"
                })
        
        return {"query": request.query, "results": results, "structured_data": structured_data}
    except Exception as e:
        print(f"DEBUG: Error in search endpoint: {e}")
        import traceback
        traceback.print_exc()
...
def web_search(request: WebSearchRequest):
    try:
        ...
        # Use Gemini with Google Search tool if available, otherwise simulate search results
        # For this implementation, we'll use Gemini to generate "search results" based on its knowledge
        # since we might not have the Google Search tool enabled in all environments.
        
        model = genai.GenerativeModel('gemini-2.5-flash', generation_config={"response_mime_type": "application/json"})
        
        prompt = f"""
        Perform a simulated web search for: "{query}"
        Provide {request.count} distinct and relevant results.
        ...
        {{
            "results": [
                {{
                    ...
                    "content": "More detailed content about this result"
                }}
            ]
        }}
        """
        ...
        try:
            structured_data = json.loads(response.text)
            search_results = structured_data.get("results", [])
        except:
            search_results = []
        ...
        for i, result in enumerate(search_results):
            results.append({
                ...
                "llm_content": result.get("content", result.get("snippet", "No description available")),
                "images": []
            })
        
        # If no results generated, provide fallback
        if not results:
             for i in range(request.count):
                results.append({
                    ...
                    "llm_content": f"Detailed information about {query}",
                    "images": []
                })
...
async def create_flashcards(request: CreateFlashcardsRequest):
    ...
    try:
        ...
        Return the flashcards as a JSON object with this structure:
        {{
            "flashcards": [
                {{
        ...
        response = generate_content_with_retry(model, flashcard_prompt)
...
async def generate_quiz(request: GenerateQuizRequest):
    try:
        ...
        response = generate_content_with_retry(model, quiz_prompt)
...
async def chat(request: ChatRequest):
    try:
        ...
        for msg in request.history:
            chat_history.append({"role": "user" if msg["sender"] == "user" else "model", "parts": [msg["text"]]})
            
        chat = model.start_chat(history=chat_history)
        
        # Simple retry logic inline for chat
        response = None
        retries = 3
        for attempt in range(retries):
            try:
                ...
                if is_quota and attempt < retries - 1:
                    ...
                    print(f"DEBUG: Chat Quota exceeded, retrying in {sleep_time:.2f}s...")
                    time.sleep(sleep_time)
                    continue
                if attempt == retries - 1 or not is_quota:
                     raise e
